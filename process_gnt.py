...
data_dir = '/Handwritten_Chinese_character_recognition'
...
for image, tagcode in read_from_gnt_dir(gnt_dir=train_data_dir):
    ...
    dir_name = '/Handwritten_Chinese_character_recognition/data/train/' + '%0.5d'%char_dict[tagcode_unicode]
...
for image, tagcode in read_from_gnt_dir(gnt_dir=test_data_dir):
    tagcode_unicode = struct.pack('>H', tagcode).decode('gb2312')
    im = Image.fromarray(image)
    dir_name = '/Handwritten_Chinese_character_recognition/data/test/' + '%0.5d'%char_dict[tagcode_unicode]
    if not os.path.exists(os.path.join(dir_name)):
        os.mkdir(os.path.join(dir_name))
